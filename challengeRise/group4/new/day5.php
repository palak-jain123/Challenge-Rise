...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['action'])) {
     ...
     echo json_encode(array("status" => "success", "message" => "$button clicked", "button" => $button));

            exit();
        }
       
        if ($_POST['action'] === 'stopTracking') {
      $button1_status = isset($_SESSION['button1_status']) ? $_SESSION['button1_status'] : "no";

            $button2_status = isset($_SESSION['button2_status']) ? $_SESSION['button2_status'] : "no";

      $elapsed_time = isset($_POST["elapsed_time"]) ? $_POST["elapsed_time"] : "0 sec";
